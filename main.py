...
# 在你的 main.py 或 run.py 的参数解析器中
parser.add_argument('--do_visualize', action='store_true', help='flag to run embedding visualization')
parser.add_argument('--strategy', type=str,default='lora', help='lora、frozen、partial_wpe_ln、full')
...
for ii in range(args.itr):
    ...
    model_optim = torch.optim.AdamW(params, lr=args.learning_rate)
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (index,batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader),disable=True):
            ...
            outputs = model(batch_x,batch_x_mark, ii, index, mode='train',epoch=epoch)
        ...
        current_run_epoch_times.append(epoch_duration)
    ...
    mses.append(mse)
    maes.append(mae)
    if current_run_epoch_times: # 确保列表不为空
        avg_epoch_time_this_run = np.mean(current_run_epoch_times)
        all_run_avg_epoch_times.append(avg_epoch_time_this_run)
        print(f"--- Run {ii+1}/{args.itr} | Average Epoch Time: {avg_epoch_time_this_run:.4f}s ---")

mses = np.array(mses)
maes = np.array(maes)
print("mse_mean = {:.4f}, mse_std = {:.4f}".format(np.mean(mses), np.std(mses)))
print("mae_mean = {:.4f}, mae_std = {:.4f}".format(np.mean(maes), np.std(maes)))
if all_run_avg_epoch_times:
    print("avg_epoch_time_mean = {:.4f}s, avg_epoch_time_std = {:.4f}s".format(
        np.mean(all_run_avg_epoch_times), np.std(all_run_avg_epoch_times)
    ))