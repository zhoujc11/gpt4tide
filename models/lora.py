...
class GPT4TS(nn.Module):
    ...
    def __init__(self, configs, device,initial_periods_hours=None):
        ...
        self.dropout_n = getattr(configs, "dropout", 0.1) # LoRA 将复用此 dropout
        ...
        if configs.is_gpt:
            ...
            # --- [新代码] LoRA (PEFT) 配置 ---
            # 1. 定义 LoRA 配置
            print("Applying LoRA (PEFT) to self.gpt2...")
            #   从 configs 读取 r (lora_r)，默认为 8
            lora_r = getattr(configs, "lora_r", 16)
            #   从 configs 读取 alpha (lora_alpha)，默认为 16
            lora_alpha = getattr(configs, "lora_alpha", 32)
            
            peft_config = LoraConfig(
                r=lora_r,
                lora_alpha=lora_alpha,
                target_modules=["wpe","c_attn"], # 针对 GPT-2 的 QKV 投影层
                lora_dropout=self.dropout_n, # 复用已有的 dropout
