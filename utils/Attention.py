...
class CrossModal(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, 
        ...
        """
        q: [B x Q x d_model] - 时间序列 patch tokens
        k: [B x K x d_model] - 文本 tokens
        v: [B x K x d_model]
        """
        scores = None
        ...
        # [关键修复] 初始化 output = q
        # ==========================================
        # 即使你不使用 PE，也必须先把 q 赋给 output，
        # 否则进入循环第一层时 output 变量不存在。
        if hasattr(self, 'use_pe') and self.use_pe:
            B, Q, D = q.shape
            if Q > self.pe_weight.size(0):
                 raise ValueError(f"Input Q length ({Q}) exceeds max_q_len ({self.pe_weight.size(0)}).")
            pe = self.pe_weight[:Q, :] 
            output = q + pe.unsqueeze(0) 
        else:
            output = q  # <--- 这里是修复的关键
        # ==========================================
