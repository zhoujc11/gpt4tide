...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    ...
    with torch.no_grad():
        for i, (index,batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader),disable=True):
            ...
            outputs = model(batch_x, batch_x_mark,itr,index, mode='valid')
...
def test(model, test_data, test_loader, args, device, itr):
    ...
    model.eval()
    with torch.no_grad():
        for i, (index,batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader),disable=True):
            ...
            outputs = model(batch_x[:, -args.seq_len:, :],batch_x_mark, itr,index, mode='test')
            ...
            # 转换器 (如 StandardScaler) 期望 (B*L, M)
            pred = test_data.inverse_transform(pred.reshape(-1, shape[-1]))
            true = test_data.inverse_transform(true.reshape(-1, shape[-1]))
    ...
    # --- [!! 新增：保存预测和真实数组 !!] ---
    # (假设您希望保存到 `./results/` 目录下，并使用 itr 命名)
    
    # 1. 定义文件夹路径
    folder_path = f'./results/itr_{itr}/' 
    
    # 2. 创建文件夹 (如果它不存在)
    os.makedirs(folder_path, exist_ok=True) 
    
    # 3. 保存 .npy 文件
    print(f"Saving preds and trues to {folder_path}...")
    ...
    print('mae:{:.4f}, mse:{:.4f}, rmse:{:.4f}, mape:{:.4f}, r2:{:.4f}'.format(mae, mse, rmse, mape,r2))
